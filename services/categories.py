...
# Список неоднозначных позиций
AMBIGUOUS_ITEMS: list[str] = [
    "сендвич", "сэндвич", "самса", "пицца", "суши", "роллы",
    "бургер", "шаурма", "донер", "беляш", "рамен", "кофе",
    "выпечка", "пирожные", "перекус", "обед"
]
...
def get_ambiguous_options(text: str) -> list[dict] | None:
    """Проверяет текст на неоднозначность.
    
    Если есть маркер «домой» или «на работу» — возвращает None (уточнение не нужно).
    Если контекст неясен — возвращает готовый список из 2 вариантов с кнопками.
    """
    text_lower = text.lower()

    # Маркеры, однозначно снимающие вопрос
    cafe_markers = ["на работу", "на работе", "с собой", "в офис", "на объект", "в кафе", "на ходу", "перекус вне"]
    home_markers = ["домой", "в дом", "дома", "для дома", "продукты", "семье", "в холодильник"]

    if any(m in text_lower for m in cafe_markers):
        return None
    if any(m in text_lower for m in home_markers):
        return None
    ...
    for item in AMBIGUOUS_ITEMS:
        if item in text_lower:
            ...
            return [
                {
                    "label": f"🍔 {item_title} на работе / в кафе",
                    "category": "Кафе, рестораны и доставка еды",
                    "subcategory": "Перекус и фастфуд",
                    "necessity": "Want",
                },
                {
                    "label": f"🏠 {item_title} домой (продукты)",
                    "category": "Еда и продукты",
                    "subcategory": "Супермаркет и рынок",
                    "necessity": "Need",
                }
            ]
    return None
...
def is_ambiguous_item(text: str) -> tuple[bool, list[str]]:
    ...
    for item in AMBIGUOUS_ITEMS:
        if item in text_lower:
            return True, ["Кафе, рестораны и доставка еды", "Еда и продукты"]
    return False, []
